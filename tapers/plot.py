...
k = -1
labels = ["old 3"]
for file in ["wonoise.raw", "wnoise.raw"]: # driven by 5.88e9
    k+=1
    LT = LTSpice_RawRead.LTSpiceRawRead(file)
    IR1 = LT.get_trace("V(one_photon)")
    IR2 = LT.get_trace("V(two_photons)")
    IR3 = LT.get_trace("V(three_photons)")
    IR4 = LT.get_trace("V(four_photons)")
    ...
    for step in range(len(steps)):
        ...
        V3 = IR3.get_wave(step)
        V4 = IR4.get_wave(step)
        ...
        ax[k].plot(abs(x.get_wave(step))*1e9-20.5, V*1e3, label="1 photon", alpha=0.8)
        ax[k].plot(abs(x.get_wave(step))*1e9-20.5, V2*1e3, label="2 photons", alpha=0.8)
        ax[k].plot(abs(x.get_wave(step))*1e9-20.5, V3*1e3, label="3 photons", alpha=0.8)
        ax[k].plot(abs(x.get_wave(step))*1e9-20.5, V4*1e3, label="4 photons", alpha=0.8)
        
        ax[k].set_xlim((0, 2))
        
        # ax[k].fill_between(abs(x.get_wave(step))*1e9, V*1e3, V2*1e3)
...
for ax1, col in zip(ax, [("No Noise", ""), 
                         ("With $\sigma=$1$\mu$A Noise", "lower right")]):
    col, loc = col
    ax1.set_title(col)
    # ax1.legend(loc=loc)
    
plt.figlegend(["1 photon", "2 photons", "3 photons", "4 photons"])

# fig.legend(["spiced model", "Actual circuit"],
#            loc="center right",
#            borderaxespad=0.1,
#            )
