...
for file in ["beautiful_malicious_circuits.raw"]:
    k+=1
    ...
    for step in range(len(steps)):
        ...
        LW = 0.7
        ax[0].plot(np.abs(x.get_wave(step))*1e9-49.8, IR1.get_wave(step)*1e6, c=cm.inferno(0.8*step/len(steps)), linewidth=LW)
        ax[1].plot(np.abs(x.get_wave(step))*1e9-49.8, IR2.get_wave(step)*1e6, c=cm.inferno(0.8*step/len(steps)), linewidth=LW)
        ...
        if step==0:
                ...
                fig.colorbar(cm.ScalarMappable(norm=mpl.colors.Normalize(vmin=11, vmax=12), cmap=truncate_colormap(cm.inferno, 0, 0.8)), cax=cax, orientation='vertical', label="Bias Current [$\mu$A]")
