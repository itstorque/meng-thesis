...
k = -1
labels = ["old 3"]
for file in ["timedomainLC_realistic_drive.raw", "timedomainLC_pulse.raw"]: # driven by 5.88e9
    k+=1
    ...
    for step in range(len(steps)):
        ...
        NAAA = ax[k].plot(abs(x.get_wave(step))*1e9, V*1e3, linewidth="2.0", label="spiced model", color="#8C001A", alpha=0.8)
        NAAB = ax[k].plot(abs(x.get_wave(step))*1e9, V2*1e3, '--', linewidth="2.0", dashes=(1, 3), label="Actual circuit", color="#FF9000", alpha=0.8)
        ...
        ax[k].set_ylabel("$R_{\mathrm{out}}$ Current (mA)")
...
for ax1, col in zip(ax, [("Sinusoidal Drive on LC filter", "lower right"), 
                         ("Pulse Drive on LC filter", "lower right")]):
    col, loc = col
    ax1.set_title(col)
    # ax1.legend(loc=loc)
