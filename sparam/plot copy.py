...
for file in ["fixed_rc_lol.raw", "fixed_exp_taper.raw"]:
    ...
    for step in range(len(steps)):
        ...
        ax[k].plot(abs(x.get_wave(step))/1e9, V2, label="spiced model", color="#8C001A", linewidth="2")
        ax[k].plot(freq[k]/1e9, 20*np.log10(S12[k]), '--', label="source $S_{22}$", color="#FF9000", linewidth="1.5", dashes=(1, 3))
