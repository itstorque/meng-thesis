...
for file in ["another_jumbled_both_squares_post.raw"]:
    ...
    IR1 = LT.get_trace("V(n002)")
    IR2 = LT.get_trace("V(u2:n3)")
    ...
    for step in range(len(steps)):
        ...
        LW = 1
        ax0.plot(np.abs(x.get_wave(step))*1e9-9.8, IR1.get_wave(step)*1e3, c=cm.inferno(0.8*step/len(steps)), linewidth=LW)
        ax1.plot(np.abs(x.get_wave(step))*1e9-9.8, IR2.get_wave(step), c=cm.inferno(0.8*step/len(steps)), linewidth=LW)
...
for label in ax1.yaxis.get_majorticklabels():
    k+=1
    print(label)
    label.set_transform(label.get_transform() + offset([0, 5, 12, 17, 17, 15][k]))
